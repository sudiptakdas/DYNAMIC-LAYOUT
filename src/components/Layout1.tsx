...
const Layout1: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    _id: '663dd6da10e0f65ddd4589d6',
    componentType: 'Label1',
    addCount: 0,
    updateCount: 0,
    title: '',
    content: '',
      ...
      const fetchData = async () => {
        try {
          const response = await axios.get(
            'https://dynamic-layout-be.vercel.app/api/663dd6da10e0f65ddd4589d6'
          );
  ...
  return (
    <div className='bg-blue-200 border border-blue-700 w-full h-full p-6'>
      <div className=' flex justify-between mb-3'>
        <div className='flex gap-4 flex-wrap'>
          ...
          <button
            ...
            onClick={handleButtonClick}
            onMouseDown={(e) => e.stopPropagation()}
            className='bg-pink-200 p-2 rounded-xl cancel-drag'
          >
            Update
          </button>
        </div>
